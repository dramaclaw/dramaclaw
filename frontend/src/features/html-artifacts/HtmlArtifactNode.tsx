...
export const HtmlArtifactNode=memo(function HtmlArtifactNode({data,selected}:NodeProps){
  ...
  return <div className={`group relative w-96 overflow-visible rounded-[var(--node-radius)] border ${canvasNodeFrameClass({selected})}`} onDoubleClick={open}>
    ...
    <div className="pointer-events-none h-56 overflow-hidden rounded-[var(--node-radius)] bg-background">{html?<iframe title={t('htmlArtifact.thumbnail')} sandbox="" referrerPolicy="no-referrer" srcDoc={srcDoc} tabIndex={-1} className="h-[672px] w-[1152px] origin-top-left scale-[0.333333] border-0 bg-white"/>:<p className="p-5 text-xs text-muted-foreground">{error||t('htmlArtifact.empty')}</p>}</div>
