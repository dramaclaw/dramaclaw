...
| P0 已实现 | 创意片头 | 选帧、六类片名/动效样式、显式节点链与可选 5 秒原片融合已落地 | 本地 compose 先裁真实片段；仅在目录明确支持图片+视频引用时建立双引用动效节点 | 基础链、融合链、本地裁片与刷新恢复均已实测；最终供应商画质与真实扣费仍需另行授权验收 |
...
截至 2026-09-24，创意片头基础链和可选 5 秒原片融合均已按上述方案实现：视频工具条打开独立
选帧弹窗；普通模式只上传关键帧并创建 `exportImageNode → imageGenNode → videoNode`，不会自动
提交图片或视频生成任务。融合模式先通过既有 `freezone_video_compose` 本地 FFmpeg 任务裁出
保留原音的真实 5 秒片段，再创建可见片段节点，并让片名设计图与片段共同引用到显式
`allReference` 动效节点。目录没有同时声明图片引用、视频引用、全能参考和 5 秒约束时，入口禁用。

片名、所选样式、提示词、模型、模式、参考顺序、5 秒区间与来源 id 都随节点保存。真实画布已验证
任务中心只出现一条本地 compose，五节点/五边创建成功，刷新后片段、双引用、模型和提示词仍在；
连接失败时弹窗保留且不留下半条工作流。关键帧继续使用 `exportImageNode` 而非 `uploadNode`：后者
没有 target handle，强行使用会让源视频到关键帧的边被统一建边规则拒绝。最终供应商生成仍保持
用户显式提交，未获费用授权前不作为本地闭环的一部分。
