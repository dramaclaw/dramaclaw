...
本轮浏览器取证只执行了低风险导航、选择和打开面板。唯一产生远端状态的动作是点击一次
“逐帧拉片”：LibTV 会**立即新增一个未提交的拉片节点**，节点数由 93 变成 94；没有点击
“开始拉片”，没有创建任务或消耗积分。为避免误删用户内容，未在远端执行删除。
...
`创意片头` 目前只确认入口存在。没有执行可能收费的生成，也未取得足以还原输入/输出协议的
面板证据，因此正确下一步是做一次受控 discovery spike，而不是按按钮名直接写功能。
...
| P0 调研 | 创意片头 | 只确认入口，语义未知 | 受控打开面板并抓请求/结果契约 | 不能从名字臆造昂贵功能 |
...
本轮目标是完成真实 UI / JS / 我方代码的取证和方案固化，不直接新增业务功能。图层分离、
