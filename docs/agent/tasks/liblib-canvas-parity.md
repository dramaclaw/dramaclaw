...
**状态**：待验收
**最后更新**：2026-09-18
**基线**：`38484897`；本地未提交；与 `origin/feat/canvas-video-reshoot-breakdown` 大面积重复
...
1. 按“导入器 / 重拍 / 续写 / 工具条”四块，将本地行为和远端分支提交 / 测试逐项对比。
2. 保留本线已有的 LibTV 实测证据与已定分叉；可复用的远端生命周期、权限、计费和恢复测试优先移植。
3. 先拆独占新增模块，再在最新主线基线上串行处理共享画布文件。
4. 配 relay 后分别端到端跑重拍与续写，验证请求、任务恢复、产物和错误文案。
...
- [ ] 这一大坨改动拆成可评审的提交：建议按「导入器 / 重拍 / 续写 / 工具条」四个 PR 切
- [ ] `docs/guides/liblib-canvas-parity.md` 目前未被 git 跟踪（`docs/guides/` 整个是未跟踪目录），
      随本轮一起提交，否则取证结果只活在这台机器上
...
- **最后完成到**：本地重拍已走到任务创建，续写未同强度实跑，导入器 / 工具条代码与测试均在。
- **下一步唯一动作**：制作本地四功能块与远端 `feat/canvas-video-reshoot-breakdown` 的行为 / 测试差异表。
